...
import winterknight.generics.Camera;
import winterknight.generics.GameRect;
import winterknight.generics.GameStatus;
import winterknight.gui.GameTextRender;
import winterknight.managers.GameManagerAudio;
import winterknight.saves.GameSaveManager;
import winterknight.scenes.backgrounds.Background;
import winterknight.scenes.backgrounds.BackgroundCastle;
import winterknight.scenes.entities.Player;
import winterknight.scenes.entities.enemies.Enemy;
import winterknight.scenes.entities.enemies.Skeleton;
import winterknight.scenes.entities.enemies.Skull;
import winterknight.scenes.entities.enemies.Thing;
import winterknight.scenes.entities.enemies.bosses.Boss;
import winterknight.scenes.objects.Life;
import winterknight.scenes.objects.Spawn;
import winterknight.scenes.objects.Sword;
import winterknight.scenes.objects.Teleport;
import winterknight.scenes.tiles.Block;
import winterknight.scenes.tiles.Floor;
import winterknight.scenes.tiles.Wall;
...
public abstract class Scene {
    ...
    public Scene(Game game, Teleport teleport, Boss boss, List<Background> backgrounds) {
        ...
        this.gravity = 0.3f; // GIẢM GRAVITY XUỐNG
    ...
    private void buildGame() {
        ...
        boolean playerSpawned = false; // THEO DÕI ĐÃ SPAWN PLAYER CHƯA
                ...
                switch (currentPixel) {
                    ...
                    default:
                        if (currentPixel == this.teleportCurrent.getColor()) {
                            this.player.setPosition(x * this.sizeBaseTiles, y * this.sizeBaseTiles - this.sizeBaseTiles); // SPAWN TRÊN TILE
                            playerSpawned = true;
                        }
        ...
        // NẾU PLAYER CHƯA ĐƯỢC SPAWN, ĐẶT LÊN FLOOR ĐẦU TIÊN
        if (!playerSpawned && !this.floors.isEmpty()) {
            Floor firstFloor = this.floors.get(0);
            this.player.setPosition(
                    firstFloor.getRect().getX(),
                    firstFloor.getRect().getY() - this.player.getRect().getHeight() - 5
            );
        }
    }
